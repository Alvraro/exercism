...
public class Satellite {
    public Tree treeFromTraversals(List<Character> preorderInput, List<Character> inorderInput) {
        return new Tree(treeFromTraversals(preorderInput, inorderInput, true));
    }
    
    private Node treeFromTraversals(List<Character> preorderInput, List<Character> inorderInput, boolean checkInput) {
        if(checkInput) {
        ...
        Node left = treeFromTraversals(leftPreorder, leftInorder, false);
        Node right = treeFromTraversals(rightPreorder, rightInorder, false);
        root.left = left;
        root.right = right;
